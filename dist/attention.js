 ...
 */
export function attentionStep(prevWindow, x, h1, kappaPrev, charOneHots, charLen, alphabetSize, weights, bias, K) {
    ...
    for (let k = 0; k < K; k++) {
        kappa[k] = kappaPrev[k] + kappaDelta[k] / 25.0;
    }
    ...
    for (let u = 0; u < charLen; u++) {
        ...
        for (let k = 0; k < K; k++) {
            ...
            phi += alpha[k] * Math.exp(-(diff * diff) / beta[k]);
        }
        // window += phi * oneHot(char_u)
        const charOffset = u * alphabetSize;
        for (let j = 0; j < alphabetSize; j++) {
            window[j] = (window[j] ?? 0) + phi * charOneHots[charOffset + j];
        }
    }
    return [window, kappa];
}
