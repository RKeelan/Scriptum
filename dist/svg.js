...
function convertAndAlign(points) {
    const allCoords = [];
    let cx = 0;
    let cy = 0;
...
function splitStrokes(allCoords) {
    const strokes = [];
    let current = [];
 ...
 */
export function strokesToSvgPaths(points) {
    if (points.length === 0)
        return [];
 ...
 */
export function strokesToCoords(points) {
    if (points.length === 0)
        return [];
    const allCoords = convertAndAlign(points);
 ...
 */
export function outlinePath(stroke, options) {
    if (stroke.length === 0)
        return "";
    if (stroke.length === 1) {
    ...
    const rawWidths = distances.map((d) => {
 ...
 */
export function strokesToOutlinePaths(points, options) {
