...
function makePoints(coords: [number, number, boolean?][]): StrokePoint[] {
...
describe("strokesToCoords", () => {
  ...
  test("converts offsets to absolute coordinates", () => {
    ...
    const first = strokes[0] as { x: number; y: number }[];
    for (let i = 1; i < first.length; i++) {
      expect((first[i] as { x: number }).x).toBeGreaterThan((first[i - 1] as { x: number }).x);
    }
  });
