...
function approxEqual(a: Float32Array, b: Float32Array) {
  ...
  for (let i = 0; i < a.length; i++) {
    expect(a[i]).toBeCloseTo(b[i] as number, 5);
  }
}
...
describe("softmax", () => {
  ...
  test("sums to 1", () => {
    ...
    const result = softmax(x);
    let sum = 0;
    for (let i = 0; i < result.length; i++) sum += result[i] as number;
  ...
  test("largest input gets largest probability", () => {
    ...
    const result = softmax(x);
    expect(result[1]).toBeGreaterThan(result[0] as number);
    expect(result[1]).toBeGreaterThan(result[2] as number);
  });
});
